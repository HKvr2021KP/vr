	...
	if(isset($_POST["photo_submit"])){
		...
		if(empty($photo_upload_error)){
			...
			if(empty($photo_upload_error)){
				...
				//suuruse muutmine
				// loome pixlikogumi ehk image objekti
				$tmp_image = null;
				if($image_file_type == "jpg"){
					$tmp_image = imagecreatefromjpeg($_FILES["file_input"]["tmp_name"]);
				}
				if($image_file_type == "png"){
					$tmp_image = imagecreatefrompng($_FILES["file_input"]["tmp_name"]);
				}

				$image_w = imagesx($tmp_image);
				$image_h = imagesy($tmp_image);

				//säilitame kuvasuhte - arvutame suuruse muutuse kordaja lähtudes kõrgusest või laiusest
				if ($image_w / $image_max_w > $image_h / $image_max_h) {
					$image_size_ratio = $image_max_w / $image_max_w ;
				} else {
					$image_size_ratio = $image_max_h / $image_max_h;
				}

				$image_new_w = round($image_w / $image_size_ratio);
				$image_new_h = round($image_h / $image_size_ratio);

				// vähendamiseks loome uue image objekti kuhu opeerime vähendatud kujutise
				$new_temp_image = imagecreatetruecolor($image_new_w, $image_new_h);
				imagecopyresampled($new_temp_image, $tmp_image, 0, 0, 0, 0, $image_new_w, $image_new_h, $image_w, $image_h);
...
<body>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
		...
		<label for="privacy_input_2">Registreeritud kasutajale </label>
		<br>

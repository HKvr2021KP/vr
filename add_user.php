<?php
  ...
  if(isset($_POST["user_data_submit"])){
	...
	if (!empty($_POST["birth_day_input"])){
		$birth_day = intval($_POST["birth_day_input"]);	//intval - täisarv
	}	else {
		$birth_day_error = "Palun vali sünnikuupäev!";
	}
	if (!empty($_POST["birth_month_input"])){
		$birth_month = intval($_POST["birth_month_input"]);	//intval - täisarv
	}	else {
		$birth_month_error = "Palun vali sünnikuu!";
	}
	if (!empty($_POST["birth_year_input"])){
		$birth_year = intval($_POST["birth_year_input"]);	//intval - täisarv
	}	else {
	...
	if (empty($birth_day_error) and empty($birth_month_error) and empty($birth_year_error)){
		if(checkdate($birth_month, $birth_day, $birth_month)) {
	...
	//viimase astme kontroll parooli-salasõna kontroll
	if(!empty($_POST["password_input"])) {
		if(strlen($_POST["password_input"])<8){		//stringi pikkus
	...
	if(empty($_POST["conformpassword_input"])) {
		$confirm_password_error = "Palen sisestage salasõna kaks korda!";
	} else {
		if($_POST["conformpassword_input"] != $_POST["password_input"]){
			$confirm_password_error = "Sisestatud salasõnad ei ole ühesusgused!";
		}
	}
	// KODUNE ÜLESANNE: kasutaja kontrollimine - meiliaadressi järgi
	
	function email_exists($email) {
		$connection = new mysqli($GLOBALS["server_host"], $GLOBALS["server_user_name"], $GLOBALS["server_password"], $GLOBALS["database"]);
		$connection -> set_charset("utf8");
		$statement = $connection -> prepare("SELECT vr21_users_email FROM vr21_users");
		echo $connection -> error;

		$statement -> bind_result($email_from_db);
		$statement -> execute();
		$emailexists = null;
	...
	if (email_exists($email) == 1){
		$email_error = "Sellise e-postiaadressiga kasutaja on juba olemas";
	}
	
	// kui vigu pole siis salvestame kasutaja
	if (empty($name_error) and empty($surname_error) and empty($birth_month_error) and empty($birth_year_error) and empty($birth_day_error) and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)){
		...
		if($notice == 1) {
			$notice = "Uus kasuaja on edukalt loodud !";
...
<html lang="et">
  ...
  <body>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <input name="conformpassword_input" type="password"><span><?php echo $confirm_password_error; ?></span><br>
